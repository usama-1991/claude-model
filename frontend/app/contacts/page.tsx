...
const tagColors: Record<string, any> = {
  ...
  Regular:  { bg:'var(--primary-light)', color:'var(--primary)' },
      ...
      <div style={{ display:'flex', height:'calc(100vh - 38px)' }}>
        ...
        <div style={{ width:300, background:'#fff', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', flexShrink:0 }}>
          <div style={{ padding:'16px 14px 10px' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              <h2 style={{ fontSize:16, fontWeight:700, color:'var(--text-primary)' }}>Contacts</h2>
              <button onClick={() => setShowAdd(true)} style={{ width:28, height:28, borderRadius:8, background:'linear-gradient(135deg, var(--primary), var(--primary-dark))', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          <div style={{ display:'flex', borderTop:'1px solid var(--border)', borderBottom:'1px solid var(--border)' }}>
            {[{l:'Total',v:list.length},{l:'VIP',v:list.filter(c=>(c.tags||[]).includes('VIP')).length},{l:'New',v:list.filter(c=>(c.tags||[]).includes('New')).length}].map(s => (
              <div key={s.l} style={{ flex:1, padding:'8px 0', textAlign:'center', borderRight:'1px solid var(--border)' }}>
                <div style={{ fontSize:16, fontWeight:700, color:'var(--text-primary)' }}>{s.v}</div>
                <div style={{ fontSize:10.5, color:'var(--text-tertiary)' }}>{s.l}</div>
              </div>
            ))}
          </div>
          <div style={{ flex:1, overflowY:'auto' }}>
            ...
            : filtered.length===0 ? (
              ...
              <div key={c.id} onClick={() => setSelected(c)}
                style={{ padding:'11px 14px', cursor:'pointer', borderBottom:'1px solid var(--border)', background:selected?.id===c.id?'var(--primary-light)':'transparent', borderLeft:selected?.id===c.id?'3px solid var(--primary)':'3px solid transparent', transition:'all 0.1s' }}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:13.5, fontWeight:600, color:'var(--text-primary)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.name||c.phone||'Unknown'}</div>
                    <div style={{ fontSize:11.5, color:'var(--text-tertiary)' }}>{c.phone||c.email||'No contact info'}</div>
                  </div>
        ...
        {selected ? (
          <div style={{ flex:1, overflowY:'auto', background:'var(--bg)' }}>
            <div style={{ background:'#fff', borderBottom:'1px solid var(--border)', padding:'20px 28px' }}>
              <div style={{ display:'flex', alignItems:'flex-start', gap:16 }}>
                <div style={{ width:58, height:58, borderRadius:'50%', background:'linear-gradient(135deg, var(--primary), var(--primary-dark))', display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, fontWeight:700, color:'#fff', flexShrink:0 }}>
                  {(selected.name||selected.phone||'?').slice(0,2).toUpperCase()}
                </div>
                <div style={{ flex:1 }}>
                  <h2 style={{ fontSize:20, fontWeight:700, color:'var(--text-primary)', letterSpacing:'-0.3px', marginBottom:4 }}>{selected.name||selected.phone||'Unknown'}</h2>
                  <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                    ...
                    {['VIP','Regular','New'].filter(t => !(selected.tags||[]).includes(t)).map(tag => (
                      <button key={tag} onClick={() => addTag(selected.id, tag)} style={{ fontSize:11, padding:'3px 9px', borderRadius:10, border:'1px dashed var(--border-strong)', background:'transparent', color:'var(--text-secondary)', cursor:'pointer' }}>+ {tag}</button>
                    ))}
                  </div>
                </div>
                <button style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', fontSize:12.5, fontWeight:600, background:'linear-gradient(135deg, var(--primary), var(--primary-dark))', color:'#fff', border:'none', borderRadius:8, cursor:'pointer' }}>
            ...
            <div style={{ padding:'20px 28px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              {/* Contact info */}
              <div style={{ background:'#fff', borderRadius:14, padding:'18px', border:'1px solid var(--border)' }}>
                <div style={{ fontSize:13, fontWeight:700, color:'var(--text-primary)', marginBottom:14 }}>👤 Contact Information</div>
                {[{icon:Phone,label:'Phone',value:selected.phone},{icon:Mail,label:'Email',value:selected.email}].map(({icon:Icon,label,value}) => (
                  <div key={label} style={{ display:'flex', gap:10, padding:'9px 0', borderBottom:'1px solid var(--border)' }}>
                    ...
                    <div>
                      <div style={{ fontSize:10.5, color:'var(--text-tertiary)', fontWeight:500, textTransform:'uppercase', letterSpacing:'0.05em' }}>{label}</div>
                      <div style={{ fontSize:13.5, color: value?'var(--text-primary)':'var(--text-tertiary)', fontWeight:500, marginTop:1 }}>{value||'Not provided'}</div>
                    </div>
                  </div>
                ))}
              </div>
              {/* Activity */}
              <div style={{ background:'#fff', borderRadius:14, padding:'18px', border:'1px solid var(--border)' }}>
                <div style={{ fontSize:13, fontWeight:700, color:'var(--text-primary)', marginBottom:14 }}>📊 Activity</div>
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  {[{l:'Total Convos',v:selected.total_conversations||0,i:'💬'},{l:'Last Contact',v:selected.last_contacted_at?new Date(selected.last_contacted_at).toLocaleDateString():'Never',i:'🕐'},{l:'Total Spent',v:selected.total_spent?`$${selected.total_spent}`:'—',i:'💰'},{l:'Channel',v:selected.whatsapp_id?'WhatsApp':selected.instagram_id?'Instagram':'Facebook',i:'📱'}].map(s => (
                    <div key={s.l} style={{ background:'var(--primary-light)', borderRadius:10, padding:'12px', border:'1px solid var(--border)' }}>
              ...
              {/* Notes */}
              <div style={{ background:'#fff', borderRadius:14, padding:'18px', border:'1px solid var(--border)', gridColumn:'1/-1' }}>
                <div style={{ fontSize:13, fontWeight:700, color:'var(--text-primary)', marginBottom:10 }}>📝 Notes</div>
                <textarea defaultValue={selected.notes||''} placeholder="Add notes about this contact…"
                  ...
                  style={{ width:'100%', minHeight:80, padding:'10px 12px', fontSize:13, border:'1.5px solid var(--border)', borderRadius:9, background:'#fafafa', fontFamily:'inherit', color:'#111', resize:'vertical', lineHeight:1.6 }}/>
              </div>
            </div>
          </div>
        ) : (
          <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:12 }}>
            <div style={{ width:56, height:56, borderRadius:'50%', background:'var(--primary-light)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26 }}>👤</div>
            <div style={{ fontSize:15, fontWeight:600, color:'var(--text-primary)' }}>Select a contact</div>
            <div style={{ fontSize:13, color:'var(--text-secondary)' }}>Contacts are auto-created from incoming messages</div>
          </div>
        )}
      </div>
      ...
      {showAdd && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={() => setShowAdd(false)}>
          <div style={{ background:'#fff', borderRadius:16, padding:'28px 32px', width:400, boxShadow:'0 20px 60px rgba(0,0,0,0.15)' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize:18, fontWeight:700, color:'var(--text-primary)', marginBottom:20 }}>Add Contact</h3>
            {[{l:'Name',k:'name',p:'Sara Ahmed'},{l:'Phone',k:'phone',p:'+92 300 0000000'},{l:'Email',k:'email',p:'email@example.com'}].map(f => (
              <div key={f.k} style={{ marginBottom:14 }}>
                ...
                <input value={(form as any)[f.k]} onChange={e => setForm(p=>({...p,[f.k]:e.target.value}))} placeholder={f.p}
                  style={{ width:'100%', padding:'10px 12px', fontSize:13.5, border:'1.5px solid var(--border)', borderRadius:9, background:'#fafafa', fontFamily:'inherit', color:'#111' }}/>
              </div>
            ))}
            <div style={{ display:'flex', gap:10, marginTop:20 }}>
              <button onClick={() => setShowAdd(false)} style={{ flex:1, padding:'10px', fontSize:13, fontWeight:600, border:'1px solid var(--border)', borderRadius:9, background:'#fff', color:'var(--text-secondary)', cursor:'pointer' }}>Cancel</button>
              <button onClick={handleAdd} disabled={saving} style={{ flex:1, padding:'10px', fontSize:13, fontWeight:600, background:'linear-gradient(135deg, var(--primary), var(--primary-dark))', color:'#fff', border:'none', borderRadius:9, cursor:'pointer' }}>
