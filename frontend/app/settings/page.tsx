    ...
    <AppLayout>
      <div style={{ padding:'24px 28px' }}>
        <div style={{ marginBottom:24 }}>
          <h1 style={{ fontSize:24, fontWeight:700, color:'var(--text-primary)', letterSpacing:'-0.5px' }}>Settings</h1>
          <p style={{ fontSize:13.5, color:'var(--text-secondary)', marginTop:3 }}>Manage your account, API connections, and preferences</p>
        </div>

        <div style={{ display:'flex', gap:2, borderBottom:'1px solid var(--border)', marginBottom:28 }}>
          {tabs.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding:'9px 18px', fontSize:13, fontWeight:tab===t?600:400, color:tab===t?'var(--primary)':'var(--text-secondary)', background:'none', border:'none', borderBottom:tab===t?'2px solid var(--primary)':'2px solid transparent', marginBottom:-1, cursor:'pointer' }}>{t}</button>
          ))}
        </div>
        ...
        <div style={{ maxWidth:580 }}>
          ...
          {/* ── Business ── */}
          {tab === 'Business' && (
            <div style={{ background:'#fff', borderRadius:14, padding:'22px', border:'1px solid var(--border)' }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ marginTop:8, display:'flex', alignItems:'center', gap:8, padding:'11px 24px', fontSize:14, fontWeight:600, background:saved?'var(--green)':'linear-gradient(135deg, var(--primary), var(--primary-dark))', color:'#fff', border:'none', borderRadius:10, cursor:'pointer', boxShadow:'0 4px 14px var(--primary-glow)', transition:'background 0.2s' }}>
          ...
          {tab === 'API Keys' && (
            <div style={{ background:'#fff', borderRadius:14, padding:'22px', border:'1px solid var(--border)' }}>
              <p style={{ fontSize:13, color:'var(--text-secondary)', marginBottom:20, lineHeight:1.6 }}>
                API keys live in your backend <code style={{ background:'var(--primary-light)', padding:'2px 6px', borderRadius:4, fontSize:12 }}>.env</code> file. Update them there directly for security.
              </p>
              {apiKeyFields.map(k => (
                <div key={k.l} style={{ marginBottom:16 }}>
                  ...
                  <div style={{ display:'flex', gap:8 }}>
                    <input defaultValue={k.v} readOnly style={{ flex:1, padding:'9px 12px', fontSize:13, border:'1.5px solid var(--border)', borderRadius:9, background:'#fafafa', fontFamily:'monospace', color:'#374151' }}/>
                    <button onClick={() => navigator.clipboard.writeText(k.v)} style={{ padding:'8px 12px', fontSize:12, border:'1px solid var(--border)', borderRadius:9, background:'#fff', cursor:'pointer', color:'var(--primary)', fontWeight:500 }}>Copy</button>
                  </div>
                  <p style={{ fontSize:11.5, color:'var(--text-tertiary)', marginTop:3 }}>{k.h}</p>
                </div>
              ))}
          ...
          {tab === 'Notifications' && (
            <div style={{ background:'#fff', borderRadius:14, padding:'22px', border:'1px solid var(--border)' }}>
              {notifDefaults.map(n => (
                <div key={n.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 0', borderBottom:'1px solid var(--border)' }}>
                  <div>
                    <div style={{ fontSize:13.5, fontWeight:500, color:'var(--text-primary)' }}>{n.label}</div>
                    <div style={{ fontSize:12.5, color:'var(--text-secondary)', marginTop:2 }}>{n.desc}</div>
                  </div>
          ...
          {tab === 'Billing' && (
            <div>
              <div style={{ background:'linear-gradient(135deg, #450a0a, var(--primary-dark))', borderRadius:14, padding:'20px 24px', marginBottom:16, color:'#fff' }}>
              ...
              {plans.map(plan => (
                <div key={plan.name} style={{ background:'#fff', borderRadius:12, padding:'16px 20px', marginBottom:10, border:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <div>
                    <div style={{ fontSize:14, fontWeight:600, color:'var(--text-primary)' }}>{plan.name}</div>
                    <div style={{ fontSize:12.5, color:'var(--text-secondary)' }}>{plan.price}</div>
                  </div>
                  <a href="mailto:admin@autoflow.ai" style={{ padding:'7px 14px', fontSize:12.5, fontWeight:600, background:'linear-gradient(135deg, var(--primary), var(--primary-dark))', color:'#fff', borderRadius:8, textDecoration:'none' }}>Request</a>
                </div>
              ))}
            </div>
          )}
